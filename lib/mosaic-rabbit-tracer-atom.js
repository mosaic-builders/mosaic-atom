...
import MosaicRabbitTracerAtomView from './mosaic-rabbit-tracer-atom-view';
...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      atom.workspace.addOpener(uri => {
        if (uri === 'atom://mosaic-rabbit-tracer-atom') {
          return new MosaicRabbitTracerAtomView();
        }
      }),
      ...
      atom.commands.add('atom-workspace', {
        'mosaic-rabbit-tracer-atom:toggleHelpPanel': () => this.toggleHelpPanel()
      }),

      // Register command that toggles debugger view
      atom.commands.add('atom-workspace', {
        'mosaic-rabbit-tracer-atom:toggleDebugger': () => this.toggleDebugger()
      }),

      // Register command that toggles debugger view
      atom.commands.add('atom-workspace', {
        'mosaic-rabbit-tracer-atom:toggleTraceMode': () => this.toggleTraceMode()
      }),

      // Destroy any MosaicRabbitTracerAtomViews when the package is deactivated
      new Disposable(() => {
        atom.workspace.getPaneItems().forEach(item => {
          if (item instanceof MosaicRabbitTracerAtomView) {
            item.destroy();
          }
        })
      })
    ...
    this.socket.on('fileName', update => {
      if (this.traceMode && update) this.updateFileName(update);
    });
    ...
    //TODO: need a toggle tracer
  },

  updateFileName(update) {
    this.fileName = update.fileName;
    this.filePath = this.examplesFolder + this.fileName;
  },

  selectLine(update) {
    console.log('RECEIVED UPDATE: ', update)
    if (update.lineNumber && update.fileName) {
      const { lineNumber, fileName } = update;
      this.fileName = fileName;
      this.filePath = this.examplesFolder + this.fileName;
      const editor = atom.workspace.getActiveTextEditor();
      console.log('PATH: ', editor.getPath())
      if (editor.getPath().includes(this.filePath)) {
  ...
  updateSelectedLine(update) {
    const editor = atom.workspace.getActiveTextEditor();
    const { selectedLine, fileName } = update;
    this.filePath = this.pathPrefix + fileName;

    if (editor.getPath().includes(this.filePath)) {
      if (selectedLine) {
        this.selectLine(editor, selectedLine);
      }
    }
  },
  ...
  deserializeMosaicRabbitTracerAtomView(serialized) {
    return new MosaicRabbitTracerAtomView();
  },
  ...
  toggleHelpPanel() {
    atom.workspace.toggle('atom://mosaic-rabbit-tracer-atom');
  },
